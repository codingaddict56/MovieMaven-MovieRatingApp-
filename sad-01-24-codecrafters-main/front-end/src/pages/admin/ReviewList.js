...
function ReviewList() {
  ...
  async function getmoviereview(){
    const response =  await axios.get(`${process.env.REACT_APP_BACKEND_URL}movie_review/get_all?page=${page}`)
    setdatas(response?.data?.datas)
    setpagination(response?.data?.pagination)
  }

  async function deletedata(){
    const response =  await axios.delete(`${process.env.REACT_APP_BACKEND_URL}movie_review/delete/${selectedData?._id}`)
      ...
      <Modal open={modal} width={300} closable={false} footer={false}>
        ...
        <div className='p-4 w-[100%]'>
            <div className='flex border-b border-gray-300 pb-2 items-center justify-between'>
                <h6 className='font-[800]'>Movie Review List ({pagination?.total}) </h6>
                {/* <div>
            ...
            <table className='mt-2 w-[100%]'>
              <thead >
                <tr className='text-[13px] font-[400] border bg-gray-100'>
                  ...
                  <th className='w-[20%] p-1 text-left border-r border-gray-300'>Movie Name </th>
                  <th className='w-[15%] p-1 text-left border-r border-gray-300'>Ratings </th>
                  <th className='w-[5%] p-1 text-left border-r border-gray-300'>Title </th>
                  <th className='w-[10%] p-1 text-left border-r border-gray-300'>Summary </th>
              ...
              <tbody>
                ...
                <tr className='text-[13px] font-[400] border'>
                  ...
                  <td className='truncate w-[15%] p-1 border-r border-gray-300'>{d?.movieId?.movieId}</td>
                  <td className='truncate w-[20%] p-1 border-r border-gray-300'>{d?.movieId?.title} </td>
                  <td className='truncate w-[5%] p-1 border-r border-gray-300'>{d?.ratings} </td>
                  <td className='truncate w-[10%] p-1 border-r border-gray-300'>{d?.title} </td>
                  <td className='truncate w-[20%] p-1 border-r border-gray-300'>{d?.summary} </td>
